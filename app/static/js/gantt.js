class Gantt {
  constructor(staticID, params) {
    ...
    this.groupBy = params.groupBy
      ? params.groupBy.split(",").map((group) => group.trim())
      : [];
    this.groupByAlias = this.groupBy
      ? params.groupByAlias
        ? params.groupByAlias.split(",").map((group) => group.trim())
        : this.groupBy
      : [];
    this.data = {};
    this.rawData = [];
  ...
  processData() {
    ...
    let sortedData = this.rawData.sort(
        (a, b) =>
          new Date(a[this.startTimeAlias]) - new Date(b[this.startTimeAlias])
      ),
      groupedData = {};
    ...
    let maxTime = this.rawData.reduce(
      (max, curr) =>
        !!curr[this.endTimeAlias]
          ? max > new Date(curr[this.endTimeAlias])
            ? max
            : new Date(curr[this.endTimeAlias])
          : max,
      new Date(0)
    );
    let minTime = this.rawData.reduce(
      (min, curr) =>
        !!curr[this.startTimeAlias]
          ? min > new Date(curr[this.startTimeAlias])
            ? new Date(curr[this.startTimeAlias])
            : min
          : min,
    ...
    for (
      let i = new Date(this.minTime.getTime());
      i <= this.maxTime;
  ...
  groupArray(result, entry, iter = 0) {
    ...
    if (!result[entry[this.groupBy[iter]]] && iter < this.groupBy.length) {
      result[entry[this.groupBy[iter]]] = {
        groupName: entry[this.groupByAlias[iter]],
      };
  ...
  buildRow(rowArr, dataIndex) {
    let totalTime = this.maxTime - this.minTime,
      compositeRows = `<div style="grid-column: 2/${
        this.divisionCount + 1
      };grid-row:1;display:flex;align-items:center"><div class="gfb-gantt-sub-row-wrapper">`;
    for (let i = 0; i < rowArr.length; i++) {
      ...
      if (
        i === 0 ||
        (rowArr[i - 1] &&
          new Date(rowArr[i - 1][this.endTimeAlias]) !== currElStart)
      ) {
        let baseTime =
            i === 0 ? this.minTime : new Date(rowArr[i - 1][this.endTimeAlias]),
      ...
      if (this.linkAlias)
        compositeRows += `<a class="gfb-gantt-row-entry" style="width:${currElRunPercent}%;" href="${
          rowArr[i][this.linkAlias]
        }" data-index="${dataIndex.join("-")}-${i}"></a>`;
      else
        compositeRows += `<div class="gfb-gantt-row-entry" style="width:${currElRunPercent}%;" data-index="${dataIndex.join(
          "-"
        )}-${i}"></div>`;
    }
  ...
  buildContent() {
    ...
    function buildContent(data, result, depth = 0, dataIndex = []) {
      for (let prop in data) {
        if (data[prop]) {
          if (prop !== "groupName") dataIndex.push(prop);

          if (typeof data[prop] === "object" && !Array.isArray(data[prop])) {
            result.push(
              `<div class="gfb-gantt-grouping-header" style="padding-left: ${
                5 + depth * 20
              }px">${data[prop].groupName}</div><div>`
            );
            depth += 1;
            ...
            result.push(
              `<div class="gfb-gantt-row" style="grid-template-columns: 100px repeat(${
                this1.divisionCount
              }, 1fr)"><div class="gfb-gantt-sidebar-header">${
                data[prop][0][this1.rowAlias]
              }</div>${this1.buildRow(data[prop], dataIndex)}</div>`
            );
          }
          dataIndex.pop();
        }
  ...
  bindHover() {
    let bindElements = document.querySelectorAll(
      `#${this.staticID} .gfb-gantt-row-entry`
    );
      ...
      bindElement.addEventListener("mouseover", (e) => {
        ...
        function getToolTipData() {
          ...
          for (let i = 0; i < indexArray.length; i++) {
            if (i === indexArray.length - 1) return dataArray[indexArray[i]];
            else dataArray = dataArray[indexArray[i]];
          }
        }
      });
  ...
  bindCollapse() {
    let bindElements = document.querySelectorAll(
      `#${this.staticID} .gfb-gantt-grouping-header`
    );
  ...
  render() {
    ...
    if (this.tooltipAlias) this.bindHover();
  }
...
function removeMultiProperty(element, properties) {
  for (property of properties) removeProperty(element, property);
}
